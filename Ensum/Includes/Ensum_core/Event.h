...
namespace Ensum
{
	namespace Utils
	{
		...
		*/
		...
		class ENSUM_CORE_EXPORT Delegate<ReturnType(ParamTypes...)>
		{
		...
		class ENSUM_CORE_EXPORT Event<ReturnType(Arguments...)>
		{
		public:
			Event() : _callbacks(nullptr)
			{
				_callbacks = new std::list<Delegate<ReturnType(Arguments...)>>;
			}
			~Event()
			{
				delete _callbacks;
